...
const SingleReportView: React.FC<{ session: AnalysisSession }> = ({ session }) => {
  ...
  const ReportPromptAccordion: React.FC<{ result: any; promptTotal: number; foundCount: number; showAllIterations: boolean; }> = ({ result, promptTotal, foundCount, showAllIterations }) => {
    ...
    return (
      <div id={`prompt-detail-${result.promptNumber}`} className="bg-[var(--acn-darkest-purple)]/40 border border-[var(--acn-dark-purple)]/50 rounded-xl overflow-hidden shadow-lg break-inside-avoid transition-all duration-300 print:bg-white print:border-gray-300 print:shadow-none print:rounded-none print:border-b-2 print:mb-4">
        <button onClick={() => setIsExpanded(!isExpanded)} className="w-full text-left p-6 border-b border-[var(--acn-dark-purple)]/50 bg-[var(--acn-darkest-purple)]/40 hover:bg-[var(--acn-darkest-purple)]/60 transition-colors flex justify-between items-start gap-4 print:bg-white print:border-gray-200 print:p-0 print:mb-2 print:block">
        ...
        <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isExpanded ? 'max-h-[8000px] opacity-100' : 'max-h-0 opacity-0'} print:!max-h-none print:!opacity-100 print:!block`}>
          <div className="p-6 bg-gray-900/50 space-y-6 print:bg-white print:p-0 print:pt-4">
            {result.iterationResults.map((iter: IterationResult, index: number) => {
                ...
                return (
                  <div key={iter.iterationNumber} className={!isBaseline ? "border-t border-gray-700/50 pt-6 animate-fade-in print:border-gray-200" : ""}>
                      ...
                      <div className={`bg-black/70 border border-gray-800 rounded-lg p-5 transition-all duration-300 print:bg-white print:border-l-4 print:border-y-0 print:border-r-0 print:border-l-gray-300 print:rounded-none print:opacity-100 print:p-0 print:pl-4`}>
                          {iter.response ? (<div className="relative"><div className={`prose prose-invert prose-sm max-w-none prose-p:leading-relaxed prose-headings:text-[var(--acn-light-purple)] prose-a:text-[var(--acn-light-purple)] hover:prose-a:text-white prose-code:text-yellow-200 ${!isTextExpanded ? 'line-clamp-3 print:line-clamp-none' : ''}`} dangerouslySetInnerHTML={renderMarkdown(iter.response)} />{!isTextExpanded && (<div className="absolute inset-x-0 bottom-0 h-8 bg-gradient-to-t from-black/70 to-transparent pointer-events-none print:hidden"></div>)}{showToggleButton && (<div className="text-right mt-2 print:hidden"><button onClick={() => toggleIterationExpansion(iter.iterationNumber)} className="text-xs text-[var(--acn-light-purple)] hover:text-white font-semibold">{isTextExpanded ? 'Show Less' : 'Show More...'}</button></div>)}</div>) : (<p className="text-gray-500 italic">No response data available.</p>)}
                      </div>
                  </div>
                );
            })}
    ...
    <div className="flex-1 flex print:block">
      <div className="flex-1 p-8 print:p-0 print:w-full">
        <div className="max-w-4xl mx-auto space-y-8 pb-12 print:space-y-6 print:pb-0">
          <section className="bg-black/50 backdrop-blur-sm border border-[var(--acn-darkest-purple)] rounded-xl p-8 shadow-lg print:bg-white print:border-gray-300 print:shadow-none print:p-0 print:border-b-2 print:rounded-none print:mb-4 print:break-inside-avoid">
