...
const getCitationScoreColor = (score: number) => {
    if (score >= 0.9) return 'bg-[var(--acn-main-purple)]/80 text-[var(--acn-lightest-purple)]';
    if (score >= 0.7) return 'bg-[var(--acn-dark-purple)]/80 text-[var(--acn-light-purple)]';
    return 'bg-[var(--acn-darkest-purple)]/80 text-[var(--acn-light-purple)]/80';
}
...
const CommonResultCard: React.FC<CommonResultCardProps> = ({ iteration, title, isBaseline, onViewMarkdown, isTargetUrlEnabled, targetUrl }) => {
    ...
    return (
        <div className="bg-gray-800/40 border border-[var(--acn-dark-purple)]/50 rounded-lg overflow-hidden">
            <button
                ...
                className="w-full flex justify-between items-center p-3 text-left hover:bg-[var(--acn-darkest-purple)]/60 transition-colors duration-200"
                aria-expanded={isExpanded}
            >
                <div className="flex flex-wrap items-center gap-3">
                    <span className="font-semibold text-gray-200 w-auto sm:w-24 text-left">{title}</span>
                    {isBaseline ? (
                        <span className="font-semibold px-2.5 py-1 rounded-full text-xs bg-[var(--acn-dark-gray)] text-white">
                            Baseline
                        </span>
                    ) : (
                        <div className="flex space-x-2">
                            ...
                            {hasSimilarityScore ? (
                                ...
                                <span className="font-semibold px-2.5 py-1 rounded-full text-xs bg-[var(--acn-dark-gray)] text-[var(--acn-gray)]">
                                    No Score
                                </span>
                            )}
                    ...
                    {iteration.response && (
                        <div>
                            <div className="flex justify-between items-center mb-2">
                                <h4 className="text-md font-semibold text-[var(--acn-light-purple)]">Gemini Response</h4>
                                <button 
                                    onClick={() => onViewMarkdown(iteration.response!)}
                                    className="text-xs bg-[var(--acn-dark-purple)] hover:bg-[var(--acn-main-purple)] text-white font-medium px-3 py-1 rounded shadow transition-colors flex items-center gap-1"
                                >
                            ...
                            <div className="text-gray-300 bg-black/50 border border-gray-800/70 p-3 rounded-md max-h-48 overflow-y-auto custom-scrollbar">
                                <p className="whitespace-pre-wrap">{iteration.response}</p>
                            </div>

                            {/* Citations Accordion */}
                            {iteration.citations && iteration.citations.length > 0 && (
                                <div className="mt-4 border border-[var(--acn-darkest-purple)]/50 rounded-lg overflow-hidden bg-gray-900/40">
                                    <div className="w-full flex justify-between items-center p-3">
                                        ...
                                        >
                                            <span className="text-sm font-semibold text-[var(--acn-light-purple)]">Sources & Citations</span>
                                            <span className="bg-[var(--acn-dark-purple)]/50 text-[var(--acn-light-purple)] text-xs px-2 py-0.5 rounded-full border border-[var(--acn-dark-purple)]/50">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex items-center cursor-pointer" onClick={(e) => { e.stopPropagation(); setShowNewOnly(!showNewOnly); }}>
                                                <span className={`text-xs mr-2 ${showNewOnly ? 'text-[var(--acn-light-purple)] font-medium' : 'text-[var(--acn-gray)]'}`}>Show New Only</span>
                                                <div className={`w-8 h-4 rounded-full relative transition-colors duration-200 ${showNewOnly ? 'bg-[var(--acn-dark-purple)]/60 border border-[var(--acn-dark-purple)]' : 'bg-gray-700 border border-gray-600'}`}>
                                            ...
                                            <button
                                                ...
                                                className="p-1.5 text-gray-400 hover:text-white hover:bg-[var(--acn-dark-purple)]/50 rounded transition-colors"
                                    ...
                                    <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isCitationsExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                                        <div className="p-0 border-t border-[var(--acn-darkest-purple)]/50">
                                            ...
                                            <div className="overflow-x-auto overflow-y-auto max-h-64 custom-scrollbar">
                                                {displayedCitations.length === 0 ? (
                                                    ...
                                                    <table className="w-full text-sm text-left text-gray-400">
                                                        <thead className="text-xs text-gray-500 uppercase bg-gray-950/60 sticky top-0 z-10 backdrop-blur-sm">
                                                            <tr>
                                                                <th className="px-4 py-2 w-16 text-center">#</th>
                                                                <th className="px-4 py-2">Source Title</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody className="divide-y divide-[var(--acn-darkest-purple)]">
                                                            {displayedCitations.map((citation, index) => {
                                                                const shouldHighlight = isTargetUrlEnabled && targetUrl && targetUrl.trim() !== '' && 
                                                                    (citation.title.toLowerCase().includes(targetUrl.trim().toLowerCase()));

                                                                return (
                                                                    <tr key={index} className={"hover:bg-[var(--acn-darkest-purple)]/50 transition-colors " + (shouldHighlight ? "bg-green-900/60" : "")}>
                                                                        ...
                                                                        <td className="px-4 py-2.5">
                                                                            <div className="flex items-center gap-2">
                                                                                <a 
                                                                                    ...
                                                                                    className="text-[var(--acn-light-purple)] hover:text-white hover:underline flex items-center gap-1 group"
                                                                                >
...
const ResultCard: React.FC<ResultCardProps> = ({ result, onViewMarkdown, isTargetUrlEnabled, targetUrl }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const { bg: summaryBg, text: summaryText } = getStatusColors(result.summaryStatus, result.summaryStatusText);
  
  return (
    <div className="bg-black/50 backdrop-blur-sm border border-[var(--acn-darkest-purple)] rounded-xl shadow-lg w-full animate-fade-in overflow-hidden">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full flex justify-between items-center p-4 text-left hover:bg-[var(--acn-darkest-purple)]/40 transition-colors duration-200"
        aria-expanded={isExpanded}
        aria-controls={`accordion-content-${result.promptNumber}`}
      >
        <div className="flex items-center space-x-4 flex-1 min-w-0">
          <span className="font-bold text-[var(--acn-light-purple)] whitespace-nowrap">Prompt #{result.promptNumber}</span>
      ...
      >
        <div className="bg-gray-900/30 px-6 border-t border-[var(--acn-darkest-purple)] space-y-6">
          <div>
            <h3 className="text-lg font-semibold text-[var(--acn-light-purple)] my-2">Full Prompt</h3>
            <p className="text-gray-300 bg-black/70 border border-gray-800 p-3 rounded-md">{result.prompt}</p>
          </div>

          <div>
            <h3 className="text-lg font-semibold text-[var(--acn-light-purple)] mb-2">Iteration Results (With Context)</h3>
          ...
          {result.noContextResult && (
            <div>
              <h3 className="text-lg font-semibold text-[var(--acn-light-purple)] mb-2 flex items-center gap-2">
